...
extends		AbstractFanEvent
{
	...
	public boolean			hasPriorityOver(EventI e)
	{
		...
		// SetMediumFan one will be executed first except for
		// SwitchOnFan ones.
	...
	public void				executeOn(AtomicModelI model)
	{
		...
		if (m.getState() == FanState.ON) {
			// a SetMedium event can only be executed when the state of the fan
			// model is in the state HIGH
			if (m.getMode() == FanMode.HIGH) {
				// then put it in the state MEDIUM
				m.setStateMode(FanState.ON, FanMode.MEDIUM);
