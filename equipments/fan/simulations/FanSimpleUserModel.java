...
extends		AtomicES_Model
{
	 ...
	 * cycles through {@code SwitchOnFan}, {@code SetHighFan}, {@code SetMediumFan}
	...
	protected void		generateNextEvent()
	{
		...
		if (current instanceof SwitchOffFan) {
			// compute the time of occurrence for the next fan usage
			Time t2 = this.computeTimeOfNextUsage(current.getTimeOfOccurrence());
			// after switching off the fan, the next event must be a
			// switch on
